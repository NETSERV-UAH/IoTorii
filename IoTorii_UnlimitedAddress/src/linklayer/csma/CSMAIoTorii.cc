...
#include "src/linklayer/csma/CSMAIoTorii.h"  //EXTRA
...
#include "src/linklayer/csma/CSMAFrameIoTorii_m.h"
//EXTRA END
...
void CSMAIoTorii::initializeMACAddress()
{
...
InterfaceEntry *CSMAIoTorii::createInterfaceEntry()
{
...
void CSMAIoTorii::handleUpperPacket(cPacket *msg)
{
    CSMAFrameIoTorii *macPkt = check_and_cast<CSMAFrameIoTorii *>(msg);
...
void CSMAIoTorii::updateStatusBackoff(t_mac_event event, cMessage *msg)
{
    switch (event) {
...
void CSMAIoTorii::flushQueue()
{
    // TODO:
    macQueue.clear();
}

void CSMAIoTorii::clearQueue()
{
    macQueue.clear();
}

void CSMAIoTorii::attachSignal(CSMAFrame *mac, simtime_t_cref startTime)
{
    simtime_t duration = mac->getBitLength() / bitrate;
    mac->setDuration(duration);
}

void CSMAIoTorii::updateStatusCCA(t_mac_event event, cMessage *msg)
{
    switch (event) {
        case EV_TIMER_CCA: {
...
void CSMAIoTorii::updateStatusTransmitFrame(t_mac_event event, cMessage *msg)
{
...
void CSMAIoTorii::updateStatusWaitAck(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void CSMAIoTorii::manageMissingAck(t_mac_event    /*event*/, cMessage *    /*msg*/)
{
...
void CSMAIoTorii::updateStatusSIFS(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void CSMAIoTorii::updateStatusTransmitAck(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void CSMAIoTorii::updateStatusNotIdle(cMessage *msg)
{
 ...
 */
void CSMAIoTorii::executeMac(t_mac_event event, cMessage *msg)
{
...
void CSMAIoTorii::updateMacState(t_mac_states newMacState)
{
 ...
 */
void CSMAIoTorii::fsmError(t_mac_event event, cMessage *msg)
{
...
void CSMAIoTorii::startTimer(t_mac_timer timer)
{
...
simtime_t CSMAIoTorii::scheduleBackoff()
{
 ...
 */
void CSMAIoTorii::handleSelfMessage(cMessage *msg)
{
...
void CSMAIoTorii::handleLowerPacket(cPacket *msg)
{
    ...
    EV << "Received frame name= " << macPkt->getName()
              << ", myState=" << macState << endl;

    if (strcmp(macPkt->getName(), "SetHLMAC") == 0){
            if (macPkt->getDestAddr().MAC == address) {
                if (!useMACAcks) {
                    EV_DETAIL << "Received a SetHLMAC packet addressed from MAC adress " << macPkt->getDestAddr().MAC <<" to me." << endl;
                    executeMac(EV_FRAME_RECEIVED, macPkt);
                }
                else{
...
cPacket *CSMAIoTorii::decapsMsg(CSMAFrame *macPkt)
{
 ...
 */
cObject *CSMAIoTorii::setUpControlInfo(cMessage *const pMsg, const MACAddress& pSrcAddr)
{
