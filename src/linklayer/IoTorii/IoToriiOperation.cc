...
#include "src/linklayer/csma/CSMAIoTorii.h"
...
void IoToriiOperation::initialize(int stage)
{
    if (stage == INITSTAGE_LOCAL) {
        ...
        myMACAddress = check_and_cast<CSMAIoTorii *>(getParentModule()->getSubmodule("mac802154"))->getMACAddress();
...
void IoToriiOperation::startCore(int core)
{
    ...
    SetHLMACFrame->setByteLength(9);  //size of IEEE802.15.4 frame format without PANIDs
    ...
    *joiningTimeTotal = *lastReceivedTime - *firstGenerationTime;
    ...
    if (!hasLoop(hlmac)){
        ...
        if (isSaved){
            ...
            EV << "Frame " << frame->getName() << " (received from dst mac address: " << frame->getDestAddr() << " )" << " frame is dropped! table is full!" << endl;
            delete frame;
        }
    }else{
...
void IoToriiOperation::handleLowerPacket(cPacket *msg)
{
    ...
    else  // Data frame
    {
        ...
        if ((hlmacTable->isMyAddress(dst)) && (counter == 1)){ //if (myAddress != HLMACAddress::UNSPECIFIED_ADDRESS){     //Data frame is mine. Send its payload to upper layer
 ...
 */   }
    ...
    if(finishCounter == numHosts){  //when last finish() is run. this is only because of numNotJoinedTotal
        ...
        fprintf(statisticsCollector,"| Number of total not joined to DODAG                              | %d\n", *numNotJoinedTotal);
