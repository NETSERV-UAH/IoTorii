...
void IPv6NeighbourDiscoveryIoTorii::processNDMessage(ICMPv6Message *msg, IPv6ControlInfo *ctrlInfo)
{
...
bool IPv6NeighbourDiscoveryIoTorii::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    throw cRuntimeError("Lifecycle operation support not implemented");
}

void IPv6NeighbourDiscoveryIoTorii::finish()
{
}

void IPv6NeighbourDiscoveryIoTorii::processIPv6Datagram(IPv6Datagram *msg)
{
    ...
    case IPv6NeighbourCacheIoTorii::INCOMPLETE:
...
IPv6NeighbourDiscoveryIoTorii::AdvIfEntry *IPv6NeighbourDiscoveryIoTorii::fetchAdvIfEntry(InterfaceEntry *ie)
{
...
IPv6NeighbourDiscoveryIoTorii::RDEntry *IPv6NeighbourDiscoveryIoTorii::fetchRDEntry(InterfaceEntry *ie)
{
...
const MACAddress& IPv6NeighbourDiscoveryIoTorii::resolveNeighbour(const IPv6Address& nextHop, int interfaceId)
{
    ...
    if (!nce || nce->reachabilityState == IPv6NeighbourCacheIoTorii::INCOMPLETE)
        return MACAddress::UNSPECIFIED_ADDRESS;

    if (nce->reachabilityState == IPv6NeighbourCacheIoTorii::STALE) {
        initiateNeighbourUnreachabilityDetection(nce);
    }
    else if (nce->reachabilityState == IPv6NeighbourCacheIoTorii::REACHABLE &&
             simTime() > nce->reachabilityExpires)
    {
        nce->reachabilityState = IPv6NeighbourCacheIoTorii::STALE;
        initiateNeighbourUnreachabilityDetection(nce);
    }
    else if (nce->reachabilityState != IPv6NeighbourCacheIoTorii::REACHABLE) {
        //reachability state must be either in DELAY or PROBE
        ASSERT(nce->reachabilityState == IPv6NeighbourCacheIoTorii::DELAY ||
                nce->reachabilityState == IPv6NeighbourCacheIoTorii::PROBE);
...
void IPv6NeighbourDiscoveryIoTorii::reachabilityConfirmed(const IPv6Address& neighbour, int interfaceId)
{
...
IPv6Address IPv6NeighbourDiscoveryIoTorii::determineNextHop(const IPv6Address& destAddr, int& outIfID)
{
...
void IPv6NeighbourDiscoveryIoTorii::initiateNeighbourUnreachabilityDetection(Neighbour *nce)
{
    ASSERT(nce->reachabilityState == IPv6NeighbourCacheIoTorii::STALE);
    ...
    nce->reachabilityState = IPv6NeighbourCacheIoTorii::DELAY;
...
void IPv6NeighbourDiscoveryIoTorii::processNUDTimeout(cMessage *timeoutMsg)
{
    ...
    if (nce->reachabilityState == IPv6NeighbourCacheIoTorii::DELAY) {
...
IPv6Address IPv6NeighbourDiscoveryIoTorii::selectDefaultRouter(int& outIfID)
{
    ...
    for (auto it = defaultRouters.begin(); it != defaultRouters.end(); ) {
        ...
        if (nce.reachabilityState != IPv6NeighbourCacheIoTorii::INCOMPLETE) {
...
void IPv6NeighbourDiscoveryIoTorii::timeoutPrefixEntry(const IPv6Address& destPrefix,
...
void IPv6NeighbourDiscoveryIoTorii::timeoutDefaultRouter(const IPv6Address& addr,
        int interfaceID)
{
    //RFC 2461: Section 6.3.5
...
void IPv6NeighbourDiscoveryIoTorii::initiateAddressResolution(const IPv6Address& dgSrcAddr,
        Neighbour *nce)
{
    const Key *nceKey = nce->nceKey;
    InterfaceEntry *ie = ift->getInterfaceById(nceKey->interfaceID);
    ...
    nce->reachabilityState = IPv6NeighbourCacheIoTorii::INCOMPLETE;
...
void IPv6NeighbourDiscoveryIoTorii::processARTimeout(cMessage *arTimeoutMsg)
{
...
void IPv6NeighbourDiscoveryIoTorii::dropQueuedPacketsAwaitingAR(Neighbour *nce)
{
    const Key *nceKey = nce->nceKey;
...
void IPv6NeighbourDiscoveryIoTorii::sendPacketToIPv6Module(cMessage *msg, const IPv6Address& destAddr,
...
void IPv6NeighbourDiscoveryIoTorii::sendQueuedPacketsToIPv6Module(Neighbour *nce)
{
    MsgPtrVector& pendingPackets = nce->pendingPackets;
...
void IPv6NeighbourDiscoveryIoTorii::processDADTimeout(cMessage *msg)
{
...
void IPv6NeighbourDiscoveryIoTorii::makeTentativeAddressPermanent(const IPv6Address& tentativeAddr, InterfaceEntry *ie)
{
...
IPv6RouterSolicitation *IPv6NeighbourDiscoveryIoTorii::createAndSendRSPacket(InterfaceEntry *ie)
{
...
void IPv6NeighbourDiscoveryIoTorii::initiateRouterDiscovery(cMessage *msg)
{
...
void IPv6NeighbourDiscoveryIoTorii::cancelRouterDiscovery(InterfaceEntry *ie)
{
...
void IPv6NeighbourDiscoveryIoTorii::processRDTimeout(cMessage *msg)
{
    InterfaceEntry *ie = (InterfaceEntry *)msg->getContextPointer();
    RDEntry *rdEntry = fetchRDEntry(ie);
...
void IPv6NeighbourDiscoveryIoTorii::processRSPacket(IPv6RouterSolicitation *rs,
...
bool IPv6NeighbourDiscoveryIoTorii::validateRSPacket(IPv6RouterSolicitation *rs,
        IPv6ControlInfo *rsCtrlInfo)
{
    bool result = true;
...
IPv6RouterAdvertisement *IPv6NeighbourDiscoveryIoTorii::createAndSendRAPacket(const IPv6Address& destAddr, InterfaceEntry *ie)
{
...
void IPv6NeighbourDiscoveryIoTorii::processRAPacket(IPv6RouterAdvertisement *ra,
...
void IPv6NeighbourDiscoveryIoTorii::processRAForRouterUpdates(IPv6RouterAdvertisement *ra,
        IPv6ControlInfo *raCtrlInfo)
{
...
void IPv6NeighbourDiscoveryIoTorii::processRAPrefixInfo(IPv6RouterAdvertisement *ra,
        InterfaceEntry *ie)
{
...
#ifndef WITH_xMIPv6
void IPv6NeighbourDiscoveryIoTorii::processRAPrefixInfoForAddrAutoConf(IPv6NDPrefixInformation& prefixInfo, InterfaceEntry *ie)
{
...
void IPv6NeighbourDiscoveryIoTorii::createRATimer(InterfaceEntry *ie)
{
    ...
    /*if ( rt6->isRouter() )
       {
        ie->ipv6()->setMinRtrAdvInterval(IPv6NeighbourDiscoveryIoTorii::getMinRAInterval()); //should be 0.07 for MIPv6 Support
        ie->ipv6()->setMaxRtrAdvInterval(IPv6NeighbourDiscoveryIoTorii::getMaxRAInterval()); //should be 0.03 for MIPv6 Support
       }*/
    // update 23.10.07 - CB
...
void IPv6NeighbourDiscoveryIoTorii::resetRATimer(InterfaceEntry *ie)
...
void IPv6NeighbourDiscoveryIoTorii::sendPeriodicRA(cMessage *msg)
{
    InterfaceEntry *ie = (InterfaceEntry *)msg->getContextPointer();
...
void IPv6NeighbourDiscoveryIoTorii::sendSolicitedRA(cMessage *msg)
{
...
bool IPv6NeighbourDiscoveryIoTorii::validateRAPacket(IPv6RouterAdvertisement *ra,
        IPv6ControlInfo *raCtrlInfo)
{
    bool result = true;
...
void IPv6NeighbourDiscoveryIoTorii::processNSPacket(IPv6NeighbourSolicitation *ns,
...
bool IPv6NeighbourDiscoveryIoTorii::validateNSPacket(IPv6NeighbourSolicitation *ns,
        IPv6ControlInfo *nsCtrlInfo)
{
    bool result = true;
...
void IPv6NeighbourDiscoveryIoTorii::processNSForTentativeAddress(IPv6NeighbourSolicitation *ns,
        IPv6ControlInfo *nsCtrlInfo)
{
    //Control Information
...
void IPv6NeighbourDiscoveryIoTorii::processNSForNonTentativeAddress(IPv6NeighbourSolicitation *ns,
        ...
        IPv6ControlInfo *nsCtrlInfo, InterfaceEntry *ie)
{
    ...
    if (entry == nullptr) {
        ...
        if (!(entry->macAddress.equals(nsMacAddr)) && !nsMacAddr.isUnspecified()) {
            ...
            entry->reachabilityState = IPv6NeighbourCacheIoTorii::STALE;
        }
    }
...
void IPv6NeighbourDiscoveryIoTorii::processNAPacket(IPv6NeighbourAdvertisement *na,
        IPv6ControlInfo *naCtrlInfo)
{
    ...
    if (neighbourEntry->reachabilityState == IPv6NeighbourCacheIoTorii::INCOMPLETE)
...
bool IPv6NeighbourDiscoveryIoTorii::validateNAPacket(IPv6NeighbourAdvertisement *na,
        IPv6ControlInfo *naCtrlInfo)
{
...
void IPv6NeighbourDiscoveryIoTorii::processNAForIncompleteNCEState(IPv6NeighbourAdvertisement *na, Neighbour *nce)
{
    ...
    if (naMacAddr.isUnspecified()) {
        ...
        if (naSolicitedFlag == true) {
            nce->reachabilityState = IPv6NeighbourCacheIoTorii::REACHABLE;
...
void IPv6NeighbourDiscoveryIoTorii::processNAForOtherNCEStates(IPv6NeighbourAdvertisement *na, Neighbour *nce)
{
    bool naRouterFlag = na->getRouterFlag();
    bool naSolicitedFlag = na->getSolicitedFlag();
        ...
        && !(naMacAddr.isUnspecified()))
    {
        ...
        if (nce->reachabilityState == IPv6NeighbourCacheIoTorii::REACHABLE) {
...
IPv6Redirect *IPv6NeighbourDiscoveryIoTorii::createAndSendRedirectPacket(InterfaceEntry *ie)
{
...
void IPv6NeighbourDiscoveryIoTorii::processRedirectPacket(IPv6Redirect *redirect,
...
void IPv6NeighbourDiscoveryIoTorii::processRAPrefixInfoForAddrAutoConf(IPv6NDPrefixInformation& prefixInfo, InterfaceEntry *ie, bool hFlag)
{
    EV_INFO << "Processing Prefix Info for address auto-configuration.\n";
    IPv6Address prefix = prefixInfo.getPrefix();
    uint prefixLength = prefixInfo.getPrefixLength();
...
void IPv6NeighbourDiscoveryIoTorii::routersUnreachabilityDetection(const InterfaceEntry *ie)
{
...
void IPv6NeighbourDiscoveryIoTorii::invalidateNeigbourCache()
{
    //Enter_Method("Invalidating Neigbour Cache Entries");
    neighbourCache.invalidateAllEntries();
}

bool IPv6NeighbourDiscoveryIoTorii::canServeWirelessNodes(InterfaceEntry *ie)
{
...
bool IPv6NeighbourDiscoveryIoTorii::isWirelessInterface(const InterfaceEntry *ie)
{
    // TODO should be a flag in the InterfaceEntry
    return strncmp("wlan", ie->getName(), 4) == 0;
}

bool IPv6NeighbourDiscoveryIoTorii::isWirelessAccessPoint(cModule *module)
{
