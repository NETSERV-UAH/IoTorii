...
Define_Module(IPv6IoTorii);

IPv6IoTorii::IPv6IoTorii() :
        curFragmentId(0)
{
}
...
#ifdef WITH_xMIPv6
IPv6IoTorii::ScheduledDatagram::ScheduledDatagram(IPv6Datagram *datagram, const InterfaceEntry *ie, MACAddress macAddr, bool fromHL) :
        datagram(datagram),
        ie(ie),
        macAddr(macAddr),
        fromHL(fromHL)
...
IPv6IoTorii::ScheduledDatagram::~ScheduledDatagram()
{
    delete datagram;
}
#endif /* WITH_xMIPv6 */

void IPv6IoTorii::initialize(int stage)
{
    if (stage == INITSTAGE_LOCAL) {
        ...
        //nd = getModuleFromPar<IPv6NeighbourDiscoveryIoTorii>(par("ipv6NeighbourDiscoveryModule"), this);
        nd = check_and_cast<IPv6NeighbourDiscoveryIoTorii *>(this->getParentModule()->getSubmodule("neighbourDiscovery"));
        //EXTRA END
...
void IPv6IoTorii::refreshDisplay() const
{
    char buf[80] = "";
    if (numForwarded > 0)
...
void IPv6IoTorii::handleMessage(cMessage *msg)
{
...
void IPv6IoTorii::endService(cPacket *msg)
{
#ifdef WITH_xMIPv6
    // 28.09.07 - CB
...
InterfaceEntry *IPv6IoTorii::getSourceInterfaceFrom(cPacket *msg)
{
    cGate *g = msg->getArrivalGate();
    return g ? ift->getInterfaceByNetworkLayerGateIndex(g->getIndex()) : nullptr;
}

void IPv6IoTorii::preroutingFinish(IPv6Datagram *datagram, const InterfaceEntry *fromIE, const InterfaceEntry *destIE, IPv6Address nextHopAddr)
{
...
void IPv6IoTorii::resolveMACAddressAndSendPacket(IPv6Datagram *datagram, int interfaceId, IPv6Address nextHop, bool fromHL)
{
...
void IPv6IoTorii::routeMulticastPacket(IPv6Datagram *datagram, const InterfaceEntry *destIE, const InterfaceEntry *fromIE, bool fromHL)
{
...
void IPv6IoTorii::localDeliver(IPv6Datagram *datagram)
{
...
void IPv6IoTorii::handleReceivedICMP(ICMPv6Message *msg)
{
...
cPacket *IPv6IoTorii::decapsulate(IPv6Datagram *datagram)
{
...
IPv6Datagram *IPv6IoTorii::encapsulate(cPacket *transportPacket, IPv6ControlInfo *controlInfo)
{
...
void IPv6IoTorii::fragmentAndSend(IPv6Datagram *datagram, const InterfaceEntry *ie, const MACAddress& nextHopAddr, bool fromHL)
{
    // hop counter check
...
void IPv6IoTorii::sendDatagramToOutput(IPv6Datagram *datagram, const InterfaceEntry *destIE, const MACAddress& macAddr)
{
...
bool IPv6IoTorii::determineOutputInterface(const IPv6Address& destAddress, IPv6Address& nextHop,
...
#ifdef WITH_xMIPv6
bool IPv6IoTorii::processExtensionHeaders(IPv6Datagram *datagram)
{
...
bool IPv6IoTorii::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    throw cRuntimeError("Lifecycle operation support not implemented");
}

// NetFilter:
void IPv6IoTorii::registerHook(int priority, INetfilter::IHook *hook)
{
    Enter_Method("registerHook()");
    hooks.insert(std::pair<int, INetfilter::IHook *>(priority, hook));
}

void IPv6IoTorii::unregisterHook(int priority, INetfilter::IHook *hook)
{
...
void IPv6IoTorii::dropQueuedDatagram(const INetworkDatagram *datagram)
{
...
void IPv6IoTorii::reinjectQueuedDatagram(const INetworkDatagram *datagram)
{
...
INetfilter::IHook::Result IPv6IoTorii::datagramPreRoutingHook(INetworkDatagram *datagram, const InterfaceEntry *inIE, const InterfaceEntry *& outIE, L3Address& nextHopAddr)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result IPv6IoTorii::datagramForwardHook(INetworkDatagram *datagram, const InterfaceEntry *inIE, const InterfaceEntry *& outIE, L3Address& nextHopAddr)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result IPv6IoTorii::datagramPostRoutingHook(INetworkDatagram *datagram, const InterfaceEntry *inIE, const InterfaceEntry *& outIE, L3Address& nextHopAddr)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result IPv6IoTorii::datagramLocalInHook(INetworkDatagram *datagram, const InterfaceEntry *inIE)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result IPv6IoTorii::datagramLocalOutHook(INetworkDatagram *datagram, const InterfaceEntry *& outIE, L3Address& nextHopAddr)
{
    for (auto & elem : hooks) {
